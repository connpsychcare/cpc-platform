...
>({
  ...
  const effectiveQueryArgs = React.useMemo(() => {
    if (!excludeIds?.length) return queryArgs;
    return { ...queryArgs, excludeIds } as TQuery;
  }, [queryArgs, excludeIds]);

  const { data, isFetching } = useQuery(effectiveQueryArgs);
  ...
  }, [data, dataKey, getOption, excludeSet]);
