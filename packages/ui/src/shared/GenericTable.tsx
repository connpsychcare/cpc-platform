...
import FilterBar, {
  ...
  type SearchByOption,
} from "@workspace/ui/shared/FilterBar";
...
> extends PaginationProps {
  ...
  onRestore?: (row: TData) => Promise<void> | void;
  canEdit?: boolean;
  canAdd?: boolean;
  ...
  setSortOrder?: (v: SortOrderType) => void;

  // Filters (replaces single filter + setFilter)
  ...
  const renderActionsMenu = (row: TData) => (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        ...
        {(onDelete || onRestore) &&
        (useDefaultActions || moreActions?.(row)?.length) ? (
          <DropdownMenuSeparator />
        ) : null}
        {onDelete && (
          <DropdownMenuItem
        ...
        <Table>
          <TableHeader className="bg-muted sticky top-0 z-10">
            <TableRow>
              ...
              {!hasActions && (
                <TableHead className="sticky right-0 z-20 w-20 bg-muted">
                  Actions
                </TableHead>
              )}
            </TableRow>
          ...
          <TableBody className="**:data-[slot=table-cell]:first:w-8">
            {data.length ? (
                ...
                <TableRow key={row.id}>
                  {columns.map((c, i) => (
                    <TableCell key={i} className={c.className}>
                      <div className={cn("min-w-0", c.wrapperCn)}>
              ...
              <TableRow>
                <TableCell
                  colSpan={columns.length + (hasActions ? 0 : 1)}
