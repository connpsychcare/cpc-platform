...
export function MediaCard({ media, onSelect }: MediaCardProps) {
  ...
  const handleDelete = async (data: MediaResponse) => {
    const ok = await confirm({
      title: "Delete media?",
      description: "Moved to trash. Permanently deleted later if not restored.",
    });
    if (!ok) return;
    await deleteAsync({ id: data.id });
  };
          ...
          <DropdownMenu>
            ...
            >
              ...
              <DropdownMenuItem
                ...
                onClick={() =>
                  openOverlay({
                    header: { title: "Update Media Form" },
                    content: (
                      <MediaForm
                        media={media}
                        isPublic
