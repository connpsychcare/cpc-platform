...
/**
 * Convert an ISO date string or Date to a YYYY-MM-DD string suitable for
 * HTML `<input type="date">` and `DatePickerField` default values.
 * Returns an empty string if the input is null / undefined / invalid.
 */
export const isoToDateInput = (value?: string | Date | null): string => {
  if (!value) return "";
  try {
