...
export function InternalConversationsList({ rolePrefix }: { rolePrefix: string }) {
  ...
  const filtered =
    filter === "unread" ? list.filter((c) => ((c as any).unreadCount ?? 0) > 0) : list;
      ...
      >
        <View className="section-wrapper gap-6 pt-6">
          <View className="gap-2">
            <Text className="font-primary text-3xl text-foreground">Messages</Text>
            <Text className="font-secondary text-sm leading-7 text-muted-foreground">
              Appointment conversations. Select one to open the chat.
            </Text>
          </View>
          ...
          >
            {filtered.length ? (
              filtered.map((conv) => {
                const patientName =
                  (conv as any).appointment?.patient?.user?.displayName ?? "Patient";
                const providerName =
                  (conv as any).appointment?.provider?.user?.displayName ?? "Provider";
                const label = `${patientName} · ${providerName}`;
                ...
                const lastTime = (conv as any).lastMessageAt ?? (conv as any).createdAt;
                ...
                return (
                  <GradientCard key={conv.id} variant="info">
                    <View className="gap-3">
                      ...
                      <Button
                        href={`${rolePrefix}/messages/${appointmentId}` as any}
              ...
              <View className="items-center gap-3 py-8">
                ...
                <Button onPress={() => setFilter("all")} variant="outline" size="sm">
                  Show all
                </Button>
              </View>
            ) : (
              <Empty>
                ...
                <EmptyHeader>
                  ...
                  <EmptyDescription>
                    Appointment chats will appear here once patients book.
                  </EmptyDescription>
                </EmptyHeader>
              </Empty>
            )}
