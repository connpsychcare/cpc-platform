...
if (isValid) {
  ...
  firebase.messaging().onBackgroundMessage(({ notification, data }) => {
    // Use the DB notification ID as the tag so we can close this specific tray
    // entry later when the user marks it as read inside the app.
...
// Close a specific browser notification when the user reads it inside the app.
