...
export default function DashboardChart({
  ...
  const historySlice = area.data.slice(
    -Math.min(historyDays, area.data.length),
  );
        ...
        <CardHeader>
          ...
          <CardAction>
            ...
            <Select
              value={historyRange}
              onValueChange={(value) =>
                setHistoryRange(value as HistoryRangeValue)
              }
            >
              <SelectTrigger
                className="flex w-40 @[700px]/card:hidden"
                size="sm"
