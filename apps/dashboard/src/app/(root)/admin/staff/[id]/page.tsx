...
import StaffPermissionsForm from "@/components/forms/StaffPermissionsForm";
import {
  Avatar,
  AvatarFallback,
  AvatarImage,
...
const sections: SectionConfig<StaffProfileResponse>[] = [
  ...
  {
    title: "Credentials",
    description: () => "Psychiatric certifications and professional credentials.",
    columns: 3,
    fields: [
      {
        label: "Credentials",
...
const Page = ({ params }: PageProps<"/admin/staff/[id]">) => {
  ...
  return (
    <div className="space-y-6">
      <GenericDetailsPage
        ...
        description={(data) =>
          `View ${data.user?.displayName}'s profile, credentials, and branch assignment.`
        }
        ...
        renderActions={(data) => (
          <Button asChild variant="outline">
            <Link href={`/admin/staff/${data.id}/caseload`}>
              View Caseload
            </Link>
          </Button>
        )}
      />
      <StaffPermissionsForm staffId={id} />
    </div>
  );
};
