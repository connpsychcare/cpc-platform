...
import { formatCompactNumber, formatDate } from "@workspace/shared/utils";
...
export default function StaffOverviewPage() {
  ...
  const assignmentTrendData =
    overview?.caseload.recentAssignments.map(({ date, count }) => ({
      ...
      assignments: count,
      sessions:
        appointmentHistoryData.find((item) => item.date === date)?.sessions ??
        0,
    })) ?? [];

  // Replaces the retired order-status pie now that commerce has been removed.
  const visitMixData = [
    { label: "Today", value: overview?.upcomingVisits.todayCount ?? 0 },
    { label: "Upcoming", value: overview?.upcomingVisits.active ?? 0 },
    { label: "Completed", value: overview?.upcomingVisits.completed ?? 0 },
  ];
  ...
  const stats: DashboardStatCardProps[] = [
    {
      ...
      bars:
        overview?.caseload.recentAssignments.map((item) => item.count) ?? [],
      icon: Users,
    },
    {
      ...
      bars:
        overview?.upcomingVisits.window.slice(-7).map((item) => item.count) ??
        [],
      icon: CalendarRange,
      tone: "success",
    },
    {
      ...
      badge: `${overview?.coordination.openConversations ?? 0} open`,
      trendLabel: "Conversations awaiting a reply from your team",
      bars: [],
      icon: MessageSquareText,
      tone: "warning",
    },
    ...
    <div className="space-y-8">
      ...
      <DashboardChart
        area={{
          title: "Session trend",
          description:
            "Assigned-patient appointment activity across the last 90 days.",
        ...
        pie={{
          title: "Visit mix",
          description: "How your assigned sessions are distributed right now.",
          data: visitMixData,
          emptyMessage: "No visit activity is available yet.",
      ...
      <section className="grid gap-6 2xl:grid-cols-[1.15fr_0.85fr]">
        <Card className="shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between gap-4">
            ...
            <Button href="/appointments" variant="outline">
              View schedule
            </Button>
          </CardHeader>
