...
} from "@workspace/contracts/traffic/dto";
...
export class TrafficService {
  ...
  async list(query: TrafficSourceQueryDto) {
    ...
    const missingIncludeIds = getMissingIncludeIds(sources, includeIds);
    const forcedSources = missingIncludeIds.length
      ? await this.prisma.trafficSource.findMany({
          where: { id: { in: missingIncludeIds } },
        })
      : [];
    const mergedSources = mergeIncludedRows(sources, forcedSources);
