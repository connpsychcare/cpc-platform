...
import type {
  ...
  MediaQueryDto,
  MediaUpdateDto,
} from "@workspace/contracts/media/dto";
...
export class MediaService {
  ...
  async findAllMedia(query: MediaQueryDto, currentUser: AuthUser) {
    const { page, limit, sortBy, sortOrder, search, searchBy, mimeType, type, includeIds = [] } =
      query;
    ...
    const missingIncludeIds = getMissingIncludeIds(medias, includeIds);
    const forcedMedias = missingIncludeIds.length
      ...
      : [];
    const mergedMedias = mergeIncludedRows(medias, forcedMedias);
  ...
  async findMedia(mediaId: string, currentUser: AuthUser) {
  ...
  async deleteMedia(mediaId: string, currentUser: AuthUser) {
    ...
    // TODO /**
    // use this await this.cloudinary.deleteFile(media.publicId, media.resourceType);
    // when we hard delete file auto our prisma extension soft delete with .delete
    // we hard delete i thing always in corn jobs with specific period of time we cna use there
    //  */
  ...
  async restoreMedia(mediaId: string, currentUser: AuthUser) {
  ...
  } as const as Prisma.MediaInclude;
}
