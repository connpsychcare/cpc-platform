...
import type { AuditLogQueryDto } from "@workspace/contracts/audit/dto";
...
export class AuditService {
  ...
  async list(query: AuditLogQueryDto) {
    ...
    const missingIncludeIds = getMissingIncludeIds(logs, includeIds);
    const forcedLogs = missingIncludeIds.length
      ? await this.prisma.auditLog.findMany({
          where: { id: { in: missingIncludeIds } },
          include: this.auditInclude,
        })
      : [];
    const mergedLogs = mergeIncludedRows(logs, forcedLogs);
